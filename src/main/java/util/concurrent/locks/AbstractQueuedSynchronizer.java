 ...
 * @since 1.5
 * @author Doug Lea
 * 提供一个框架来实现阻塞锁和相关的同步器(信号量、事件等)，这些同步器依赖于先进先出(FIFO)等待队列。这个类被设计成大多数依赖单个原子int值来表示状态的同步器的有用基础。子类必须定义改变这种状态的受保护方法，并定义该状态在获取或释放该对象时的含义。考虑到这些，这个类中的其他方法执行所有队列和阻塞机制。子类可以维护其他的状态字段，但只有通过使用方法getState、setState和compareAndSetState的原子更新的int值来跟踪同步。
子类应该定义为非公共的内部助手类，用于实现其封闭类的同步属性。类AbstractQueuedSynchronizer不实现任何同步接口。相反，它定义了一些方法，如acquireInterruptibly，它可以被具体的锁和相关的同步器调用，以实现它们的公共方法。
该类支持默认的独占模式和共享模式。当在独占模式下获取时，其他线程尝试获取无法成功。多个线程的共享模式可以(但不需要)成功。这个类不“理解”这些差异，除非在机械意义上，当共享模式获得成功时，下一个等待线程(如果存在)还必须确定它是否也能获得。在不同模式下等待的线程共享相同的FIFO队列。通常，实现子类只支持其中一种模式，但两者都可以发挥作用，例如在ReadWriteLock中。只支持独占模式或仅支持共享模式的子类不需要定义支持未使用模式的方法。
这个类定义了一个嵌套的AbstractQueuedSynchronizer。ConditionObject类,可以用作条件由子类实现支持独占模式的方法isHeldExclusively报告同步是否只对当前线程持有,方法释放与当前调用getState值完全释放该对象,并获得,鉴于这个保存的状态值,最终获得的该对象恢复到以前的状态。否则，任何AbstractQueuedSynchronizer方法都不会创建这样的条件，因此如果不能满足这个约束，就不要使用它。AbstractQueuedSynchronizer的行为。ConditionObject当然取决于它的同步器实现的语义。
该类为内部队列提供检查、插装和监视方法，以及条件对象的类似方法。可以使用AbstractQueuedSynchronizer将它们按照需要导出到类中，以实现它们的同步机制。
这个类的序列化只存储底层的原子整数保持状态，因此反序列化对象具有空线程队列。需要序列化的典型子类将定义一个readObject方法，在反序列化时将其恢复到已知的初始状态。
 */
//提供一个框架来实现阻塞锁和相关的同步器(信号量、事件等)，这些同步器依赖于先进先出(FIFO)等待队列。这个类被设计成大多数依赖单个原子int值来表示状态的同步器的有用基础
//    该类支持默认的独占模式和共享模式
public abstract class AbstractQueuedSynchronizer
     ...
     * with initial synchronization state of zero.创建一个初始同步状态为零的新的AbstractQueuedSynchronizer实例。
     */
    ...
    static final class Node {
         ...
         * unconditionally propagate waitStatus值表示下一个默认共享应该无条件传播。
         */
     ...
     * 旋转速度比使用计时停车更快的纳秒数。粗略的估计足以在很短的时间内提高响应能力。
     */
...
//    释放共享锁
     ...
     * 为队列中已经存在的线程获取独占的不可中断模式。用于条件等待方法和获取方法。
     */
...
//    获取共享锁
     ...
     * 如果同步仅针对当前(调用)线程保持，则返回true。此方法在每次调用非等待的AbstractQueuedSynchronizer时调用。ConditionObject方法。(等待方法代替调用release。)
    默认实现抛出UnsupportedOperationException。此方法仅在AbstractQueuedSynchronizer中在内部调用。如果不使用条件，则不需要定义ConditionObject方法。
     */
    protected boolean isHeldExclusively() {
        throw new UnsupportedOperationException();
    }
...
//    释放共享锁
    ...
    /**
     * Version of getFirstQueuedThread called when fastpath fails当fastpath失败时调用的getFirstQueuedThread的版本
     */
     ...
     * uses this synchronizer as its lock.查询给定的ConditionObject是否使用这个同步器作为其锁。
     *
     ...
     * 作为锁实现基础的抽象队列同步器的条件实现。
     该类的方法文档描述的是机制，而不是锁和条件用户的行为规范。这个类的导出版本通常需要附带描述条件语义的文档，这些条件语义依赖于相关的AbstractQueuedSynchronizer。
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        /**
         * Adds a new waiter to wait queue.添加一个新的服务员等待队列。
        ...
        /**
         * Removes and transfers all nodes.删除并传输所有节点。
         * @param first (non-null) the first node on condition queue
         */
         ...
         * owning lock.移动最长等待的线程，如果存在，则从等待队列的等待队列到拥有锁的等待队列。
         *
         ...
         * the wait queue for the owning lock.将此条件的所有线程从等待队列移动到拥有锁的等待队列。
         *
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
         *         returns {@code false}
         */
         ...
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
         *         returns {@code false}
         *         查询是否有线程在此条件下等待。实现hasWaiters(AbstractQueuedSynchronizer.ConditionObject)。
         */
         ...
         * @throws IllegalMonitorStateException if {@link #isHeldExclusively}
         *         returns {@code false}
         *         返回在此条件下等待的线程数的估计值。实现getWaitQueueLength(AbstractQueuedSynchronizer.ConditionObject)。
         */
         ...
         *         returns {@code false}
         *
        返回包含可能在此条件下等待的线程的集合。实现getWaitingThreads(AbstractQueuedSynchronizer.ConditionObject)。
         */
     ...
     * 支持compareAndSet设置。我们需要在这里本地实现这一点:为了允许将来的增强，我们不能显式地将AtomicInteger子类化，否则这将是高效和有用的。因此，作为较小的弊端，我们采用hotspot intrinsic API实现。当我们处理它的时候，我们对其他的CASable字段也做了同样的处理(这可以用原子字段更新程序来完成)。
     */
