 ...
 *           一种基于反射的实用程序，允许对指定类的指定volatile int字段进行原子更新。这个类是为在原子数据结构中使用而设计的，在原子数据结构中，同一节点的多个字段独立地受原子更新的影响。
...
public abstract class AtomicIntegerFieldUpdater<T> {
    ...
    /**
     * Standard hotspot implementation using intrinsics.使用intrinsic的标准hotspot实现。
     */
    private static final class AtomicIntegerFieldUpdaterImpl<T>
         ...
         * Equivalent to the inaccessible: first.isAncestor(second).如果可以在第一个classloader的委托链中找到第二个classloader，则返回true。相当于不可访问的:first. is祖(second)。
         */
