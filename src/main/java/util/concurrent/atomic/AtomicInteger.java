 ...
 * @since 1.5
 * @author Doug Lea
 * 可以以原子方式更新的int值。看到java . util . concurrent。描述原子变量属性的原子包规范。AtomicInteger用于诸如原子递增计数器之类的应用程序，不能用于替换整数。但是，这个类确实扩展了Number，允许处理基于数字的类的工具和实用程序进行统一访问。
*/
public class AtomicInteger extends Number implements java.io.Serializable {
    ...
    /**
     * Creates a new AtomicInteger with the given initial value.使用给定的初始值创建一个新的AtomicInteger。
     *
    ...
    /**
     * Creates a new AtomicInteger with initial value {@code 0}.创建一个初始值为0的新AtomicInteger。
     */
    ...
    /**
     * Returns the value of this {@code AtomicInteger} as an {@code int}.返回这个AtomicInteger的值作为int类型。
     */
    public int intValue() {
        return get();
    }
     ...
     * @jls 5.1.2 Widening Primitive Conversions将这个AtomicInteger的值返回到一个扩展的原始转换之后的很长时间。
     */
     ...
     * after a widening primitive conversion.
     * @jls 5.1.2 Widening Primitive Conversions在扩展原语转换之后，返回原子整数的值作为浮点数。
     */
     ...
     * after a widening primitive conversion.
     * @jls 5.1.2 Widening Primitive Conversions在扩展原语转换之后，返回原子整数的值作为双精度。
     */
