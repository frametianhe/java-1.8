 ...
 * @since 1.8
 * @author Doug Lea
 * 一个或多个变量一起维护使用提供的函数更新的运行长值。当更新(方法积累)在线程之间进行争用时，变量集可以动态增长以减少争用。方法get(相当于longValue)在维护更新的变量之间返回当前值。
当多个线程更新用于收集统计信息而不是用于细粒度同步控制的公共值时，此类通常比AtomicLong更可取。在低更新争用下，这两个类具有相似的特征。但在高争用的情况下，该类的预期吞吐量显著提高，代价是空间消耗更高。
线程内或线程间的积累顺序没有保证，也不能依赖于它，因此该类只适用于那些积累顺序无关紧要的函数。所提供的累加器函数应该是无副作用的，因为当尝试的更新由于线程之间的争用而失败时，可以重新应用它。函数的第一个参数是当前值，第二个参数是给定的更新。例如，为了保持运行最大值，您可以提供Long: max和Long。MIN_VALUE身份。
类LongAdder为维护计数和和和的常见特殊情况提供类似于该类的功能。调用new LongAdder()等同于新的LongAccumulator((x, y) -> x + y, 0L。
...
public class LongAccumulator extends Striped64 implements Serializable {
    ...
    /**
     * Updates with the given value.更新给定的值。
     *
     * @param x the value
     */
     ...
     * after a widening primitive conversion.在扩展的原语转换之后返回当前值作为浮点数。
     */
     ...
     * after a widening primitive conversion.在扩展的原语转换之后，返回当前值为双值。
     */
    ...
    private static class SerializationProxy implements Serializable {
         ...
         * held by this proxy.返回一个长累加器对象，该对象的初始状态由该代理持有。
         */
