 ...
 * (i.e., those with more than 64 elements).枚举集的私有实现类，用于“巨型”枚举类型(例如。，包含64个以上元素)。
 *
 * @author Josh Bloch
 * @since 1.5
 * @serial exclude
...
class JumboEnumSet<E extends Enum<E>> extends EnumSet<E> {
     ...
     * in this set.这个集合的位向量表示。这个数组的第j个元素的第i位表示这个集合中存在的宇宙[64*j +i]。
     */
    private long elements[];

    // Redundant - maintained for performance冗余-维护性能。
    private int size = 0;
     ...
     * ConcurrentModificationException}.返回该集合中包含的元素的迭代器。迭代器遍历其自然顺序中的元素(这是声明枚举常量的顺序)。返回的迭代器是一个“弱一致性”迭代器，不会抛出ConcurrentModificationException。
     *
    ...
    private class EnumSetIterator<E extends Enum<E>> implements Iterator<E> {
         ...
         * of the set not yet returned by this iterator.一个位向量，表示该迭代器尚未返回的集合的当前“word”中的元素。
         */
        long unseen;

        /**
         * The index corresponding to unseen in the elements array.元素数组中未见的索引。
         */
        int unseenIndex = 0;

        /**
         * The bit representing the last element returned by this iterator
         * but not removed, or zero if no such element exists.表示此迭代器返回但未删除的最后一个元素的位，如果不存在此类元素，则为0。
         */
        long lastReturned = 0;

        /**
         * The index corresponding to lastReturned in the elements array.元素数组中与lastreturn对应的索引。
         */
    ...
    /**
     * Returns <tt>true</tt> if this set contains the specified element.如果该集合包含指定的元素，则返回true。
     *
    ...
    /**
     * Adds the specified element to this set if it is not already present.将指定的元素添加到该集合中，如果该集合不存在。
     *
    ...
    /**
     * Removes the specified element from this set if it is present.如果指定的元素存在，则从该集合中删除它。
     *
     ...
     * in the specified collection.如果该集合包含指定集合中的所有元素，则返回true。
     *
     ...
     * the specified collection.从这个集合中删除包含在指定集合中的所有元素。
     *
     ...
     * specified collection.只保留该集合中包含在指定集合中的元素。
     *
     ...
     * this set.将指定的对象与此集合进行比较，以获得相等性。如果给定对象也是一个集合，则返回true，这两个集合具有相同的大小，并且给定集合的每个成员都包含在这个集合中。
     *
    ...
    /**
     * Recalculates the size of the set.  Returns true if it's changed.重新计算集合的大小。如果更改，返回true。
     */
