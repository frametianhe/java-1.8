 ...
 * 一个浮动缓冲。
该类定义了浮动缓冲区上的四种操作:
绝对的和相对的获取和放置方法，读取和写入单个浮点数;
相对块获取方法，将连续的浮点序列从缓冲区传输到数组;和
将浮点数连续序列从浮点数组或其他浮点缓冲区传输到此缓冲区的相对散装方法;和
用于压缩、复制和分割浮动缓冲区的方法。
浮动缓冲区可以通过分配(分配为缓冲区的内容分配空间)、将现有的浮动数组封装到缓冲区中或者创建现有字节缓冲区的视图来创建。
与字节缓冲区一样，浮点缓冲区也可以是直接的，也可以是非直接的。通过此类的wrap方法创建的浮动缓冲区是非直接的。作为字节缓冲区的视图创建的浮点缓冲区将是直接的，只有当字节缓冲区本身是直接的。浮动缓冲区是否直接可以通过调用isDirect方法来确定。
    ...
    implements Comparable<FloatBuffer>
{
    ...
    //
    final float[] hb;                  // Non-null only for heap buffers非空仅用于堆缓冲区
    final int offset;
     ...
     *          If this buffer is read-only
     *          相对批量放置方法(可选操作)。
    该方法将在给定源缓冲区中剩余的浮点数转移到这个缓冲区中。如果源缓冲区中剩余的浮点数多于此缓冲区中的浮点数，也就是说，如果src.remaining() >存留()，则不会传输任何浮点数，并抛出BufferOverflowException。
    否则，该方法将n = src.remaining()从给定的缓冲区复制到这个缓冲区，从每个缓冲区的当前位置开始。然后两个缓冲区的位置都增加了n。
