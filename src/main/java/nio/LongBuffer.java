 ...
 * 很长一段缓冲。
该类定义了长缓冲区上的四类操作:
绝对的和相对的获取和放置方法，读和写单长;
相对块获取方法，将连续的长序列从缓冲区传输到数组中;和
将长数组或其他长缓冲区中的连续长序列传输到此缓冲区的相对散装方法;和
用于压缩、复制和分割长缓冲区的方法。
可以通过分配(分配为缓冲区的内容分配空间)、将现有的长数组封装到缓冲区中或者创建现有字节缓冲区的视图来创建长缓冲区。
与字节缓冲区一样，长缓冲区也可以是直接的，也可以是非直接的。通过这个类的wrap方法创建的一个长缓冲区将是非直接的。当且仅当字节缓冲区本身是直接的时，作为字节缓冲区的视图创建的长缓冲区将是直接的。长缓冲区是否直接可以通过调用isDirect方法来确定。
    ...
    implements Comparable<LongBuffer>
{
    ...
    //
    final long[] hb;                  // Non-null only for heap buffers 非空仅用于堆缓冲区
    final int offset;
     ...
     *          相对的get方法。读取缓冲区当前位置的long，然后增加该位置。
     */
     ...
     *          parameters do not hold
     *          相对批量get方法。
    该方法将缓冲区中的长数据传输到给定的目标数组中。如果缓冲区中剩余的长时间比满足请求所需的长时间要少，也就是说，如果长度>剩余()，那么就不会传输长时间，并抛出BufferUnderflowException。
    否则，该方法将该缓冲区的长度从长数据复制到给定的数组中，从该缓冲区的当前位置和数组中的给定偏移量开始。然后，这个缓冲区的位置会随着长度的增加而增加。
     ...
     * @throws  ReadOnlyBufferException
     *          If this buffer is read-only
     *          这个方法将在给定的源缓冲区中保留的长字符传输到这个缓冲区。如果源缓冲区中剩余的longs多于此缓冲区中的longs，即如果src.remaining() >存留()，则不会传输longs，并抛出BufferOverflowException。
    否则，该方法将n = src.remaining()从给定缓冲区复制到该缓冲区，从每个缓冲区的当前位置开始。然后两个缓冲区的位置都增加了n。
