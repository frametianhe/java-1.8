 ...
 * int缓冲区。
这个类定义了在int缓冲区上的四种操作:
绝对的和相对的获取和放置方法，读取和写入单个ints;
相对块获取方法，将连续的ints序列从缓冲区传输到数组;和
将连续的int序列从int数组或其他int缓冲区传输到此缓冲区的相对散装方法;和
用于压缩、复制和分割int缓冲区的方法。
可以通过分配(分配为缓冲区的内容分配空间)、将现有的Int数组封装到缓冲区中或者创建现有字节缓冲区的视图来创建Int缓冲区。
与字节缓冲区一样，int缓冲区也可以是直接的，也可以是非直接的。通过此类的wrap方法创建的int缓冲区将是非直接的。当且仅当字节缓冲区本身是直接的时，作为字节缓冲区的视图创建的int缓冲区将是直接的。是否可以通过调用isDirect方法来确定int缓冲区是否直接。
这个类中没有返回值的方法被指定为返回被调用的缓冲区。这允许对方法调用进行链接。
 */
    ...
    implements Comparable<IntBuffer>
{
    ...
    //
    final int[] hb;                  // Non-null only for heap buffers 非空仅用于堆缓冲区
    final int offset;
     ...
     * and its {@link #arrayOffset array offset} will be zero.分配一个新的int缓冲区。
     新缓冲区的位置将为零，其限制将为其容量，其标记将未定义，其每个元素将初始化为零。它将有一个背景数组，其数组偏移量为零。
     *
     * @param  capacity
     ...
     * its {@link #arrayOffset array offset} will be zero.  </p>将int数组包装到缓冲区中。
     新的缓冲区将由给定的int数组支持;也就是说，对缓冲区的修改将导致对数组的修改，反之亦然。新缓冲区的容量将是数组。长度，其位置将偏移，其限制将偏移+长度，其标记将未定义。它的背景数组将是给定的数组，它的数组偏移量为零。
     *
     * @param  array
     ...
     * be zero.  </p>将int数组包装到缓冲区中。
     新的缓冲区将由给定的int数组支持;也就是说，对缓冲区的修改将导致对数组的修改，反之亦然。新的缓冲区的容量和限制将是数组。长度，其位置为零，其标记未定义。它的背景数组将是给定的数组，它的数组偏移>将是零。
     *
     * @param  array
     ...
     *          parameters do not hold
     *          相对批量get方法。
    这个方法将ints从缓冲区传输到给定的目标数组。如果缓冲区中剩余的ints比满足请求所需的ints少，即如果长度>剩余()，则不传输ints，并抛出BufferUnderflowException。
    否则，该方法将该缓冲区中的长度ints复制到给定的数组中，从该缓冲区的当前位置和数组中的给定偏移量开始。然后，这个缓冲区的位置会随着长度的增加而增加。
    换句话说，这是对src表单方法的调用。get(dst, off, len)与循环具有完全相同的效果
    除了它首先检查缓冲区中是否有足够的ints，并且它可能更有效。
     */
     ...
     *          If this buffer is read-only
     *          相对批量放置方法(可选操作)。
    此方法将保留在给定源缓冲区中的int数传输到此缓冲区。如果源缓冲区中剩余的ints比该缓冲区中的ints要多，也就是说，如果src.remaining() >存留()，那么就不会传输ints，并抛出BufferOverflowException。
    否则，此方法将从给定缓冲区复制n = src.remaining() ints到此缓冲区，从每个缓冲区的当前位置开始。然后两个缓冲区的位置都增加了n。
