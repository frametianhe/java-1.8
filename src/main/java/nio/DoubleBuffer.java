...
这个类定义了双重缓冲区的四类操作:
绝对的和相对的获取和放置方法，读写单双;
相对块获取方法，将连续的双精度浮点序列从缓冲区传输到数组;和
将双数列连续序列从双数组或其他双缓冲区传输到此缓冲区的相对散装方法;和
用于压缩、复制和切片双重缓冲区的方法。
可以通过分配(分配为缓冲区的内容分配空间)、将现有的双数组封装到缓冲区中或者创建现有字节缓冲区的视图来创建双缓冲区。
与字节缓冲区一样，双缓冲区也可以是直接的，也可以是非直接的。通过此类的wrap方法创建的双缓冲区将是非直接的。当且仅当字节缓冲区本身是直接的时，作为字节缓冲区的视图创建的双缓冲区将是直接的。双缓冲是否直接可以通过调用isDirect方法来确定。
    ...
    implements Comparable<DoubleBuffer>
{
    ...
    //
    final double[] hb;                  // Non-null only for heap buffers非空仅用于堆缓冲区
    final int offset;
     ...
     *          If this buffer is read-only
     *          相对批量放置方法(可选操作)。
    此方法将给定源缓冲区中剩余的双精度浮点数传输到此缓冲区。如果源缓冲区中剩余的双精度浮点数多于此缓冲区中的双精度浮点数，即如果src.remaining() >存留()，则不传输双精度浮点数，并抛出BufferOverflowException。
    否则，此方法将从给定缓冲区复制n = src.remaining()到此缓冲区，从每个缓冲区的当前位置开始。然后两个缓冲区的位置都增加了n。
