 ...
 * 短暂的缓冲。
该类定义了短缓冲区上的四类操作:
绝对的和相对的获取和放置方法，读和写单条短裤;
相对块获取方法，将这个缓冲区中的连续的短路序列传输到一个数组中;和
相对散装的方法，将短距离阵列或其他短缓冲区内的连续的短裤序列传递到这个缓冲区;和
压缩、复制和分割短缓冲区的方法。
可以通过分配(分配为缓冲区的内容分配空间)、将现有的短数组封装到缓冲区中或者创建现有字节缓冲区的视图来创建短缓冲区。
与字节缓冲区一样，短缓冲区也可以是直接的，也可以是非直接的。通过此类的wrap方法创建的短缓冲区是非直接的。当且仅当字节缓冲区本身是直接的时，作为字节缓冲区视图创建的短缓冲区将是直接的。短缓冲区是否直接可以通过调用isDirect方法来确定。
    ...
    implements Comparable<ShortBuffer>
{
    ...
    //
    final short[] hb;                  // Non-null only for heap buffers非空仅用于堆缓冲区
    final int offset;
     ...
     *          If this buffer is read-only
     *          相对批量放置方法(可选操作)。
    此方法将给定源缓冲区中剩余的短路转移到此缓冲区。如果源缓冲区中剩余的短路比这个缓冲区中的多，也就是说，如果src.remaining() >存留()，那么就不会传输任何短路，并抛出一个BufferOverflowException。
    否则，该方法将给定缓冲区中的n = src.remaining()短路复制到此缓冲区，从每个缓冲区的当前位置开始。然后两个缓冲区的位置都增加了n。
    ...
    /**
     * Tells whether or not this short buffer is direct.告诉这个短缓冲区是否为直接缓冲区。
     *
