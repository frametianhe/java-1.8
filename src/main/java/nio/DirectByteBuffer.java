    ...
    implements DirectBuffer
{
    ...
    // Base address, used in all indexing calculations 基本地址，用于所有索引计算
    // NOTE: moved up to Buffer.java for speed in JNI GetDirectBufferAddress 注意:移动到缓冲区。java表示JNI GetDirectBufferAddress中的速度
    //    protected long address;

    // An object attached to this buffer. If this buffer is a view of another 附加到此缓冲区的对象。如果这个缓冲区是另一个的视图
    // buffer then we use this field to keep a reference to that buffer to 然后我们使用这个字段来保存对那个缓冲区的引用。
    // ensure that its memory isn't freed before we are done with it. 在我们使用它之前，确保它的内存没有被释放。
    ...
    // memory. A given arbitrary object may also be attached to the buffer.的块来构造直接的ByteBuffer
//内存。给定的任意对象也可以附加到缓冲区。
    //
