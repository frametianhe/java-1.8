 ...
 * are associated with different providers.当源和目标与不同的提供程序相关联时，帮助器类支持复制或移动文件。
 */
...
class CopyMoveHelper {
    ...
    /**
     * Parses the arguments for a file copy operation.解析文件复制操作的参数。
     */
     ...
     * for copying the file when a move is implemented as copy + delete.将给定的选项数组转换为将文件移动到适合复制文件的选项，当移动实现为copy + delete时。
     */
     ...
     * providers 当源和目标与不同的提供者相关联时，可以使用简单的复制
     */
     ...
     * associated with different providers 当源和目标与不同的提供者相关联时，简单移动实现为copy+delete
     */
