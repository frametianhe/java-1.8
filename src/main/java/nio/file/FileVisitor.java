 ...
 *
 * @since 1.7
 * 客人的文件。该接口的实现提供给文件。使用walkFileTree方法访问文件树中的每个文件。
 * 用法示例:假设我们要删除一个文件树。在这种情况下，应该在删除目录中的条目之后删除每个目录。
...
public interface FileVisitor<T> {
     ...
     *          在访问目录中的条目之前为目录调用。
    如果该方法返回CONTINUE，则访问目录中的条目。如果该方法返回SKIP_SUBTREE或skip_sibling，则不会访问目录中的条目(以及任何子项)。
     */
     ...
     * @throws  IOException
     *          if an I/O error occurs
     *          为无法访问的文件调用。如果无法读取文件的属性、文件是无法打开的目录等原因，则调用此方法。
     */
     ...
     * @throws  IOException
     *          if an I/O error occurs
     *          已访问了目录中的条目及其所有子项后的目录。当目录的迭代提前完成时(通过visitFile方法返回skip_sibling，或者在遍历目录时出现I/O错误)，也会调用此方法。
     */
