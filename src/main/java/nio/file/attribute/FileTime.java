 ...
 * 表示文件的时间戳属性的值。例如，它可能表示文件最后一次修改、访问或创建的时间。
该类的实例是不可变的。
 */
    ...
    implements Comparable<FileTime>
{
     ...
     * in this scenario.
     * 解释值的粒度单位。如果从瞬间转换此文件时间，则在此场景中不使用值和单元对。
     */
    private final TimeUnit unit;

    /**
     * The value since the epoch; can be negative.时代以来的价值;可以是负的。
     */
    private final long value;

    /**
     * The value as Instant (created lazily, if not from an instant)作为瞬发的价值(即使不是从瞬间产生的，也是惰性的)
     */
    private Instant instant;

    /**
     * The value return by toString (created lazily)通过toString返回的值(延迟创建)
     */
     ...
     * @since 1.8
     * 返回一个文件时间，该文件时间表示时间线上与提供的即时对象相同的时间点。
     */
     ...
     *
    返回给定粒度单元的值。
    从粗粒度到长粒度的转换。如果是负数或较长的值。MAX_VALUE如果积极。
     */
     ...
     * @since 1.8
     * 将此文件时间对象转换为瞬间。
    转换创建一个表示与此文件时间线上相同的时间点的瞬间。
    FileTime可以将时间线上的点存储在未来，而不是瞬间。从这样的时间点转换到瞬间就饱和了。越早越好。分钟或即时。MAX如果比实例晚的话。
     */
