 ...
 *
 * @since 1.7
 * 文件属性视图，提供遗留“DOS”文件属性的视图。这些属性由文件系统支持，例如在消费设备中常用的文件分配表(FAT)格式。
DosFileAttributeView是一个BasicFileAttributeView，它还支持对DOS属性标记集的访问，这些标记用于指示文件是否为只读、隐藏、系统文件或存档。
getAttribute方法可以用来读取任何这些属性，或者BasicFileAttributeView定义的任何属性，就像调用readAttributes()方法一样。
setAttribute方法可用于更新文件的最后修改时间、最后访问时间或创建BasicFileAttributeView定义的时间属性。它还可以用于更新DOS属性，就像分别调用setReadOnly、setHidden、setSystem和setArchive方法一样。
 */
    ...
    extends BasicFileAttributeView
{
     ...
     * have the name {@code "dos"}.返回属性视图的名称。这种类型的属性视图的名称为“dos”。
     */
    @Override
    String name();
     ...
     *          更新只读属性的值。
    如果可以将属性更新为与其他文件系统操作相关的原子操作，则它是特定于实现的。例如，实现可能需要读取DOS属性的现有值，以便更新此属性。
     */
