 ...
 * @since 1.7
 * 访问控制列表(ACL)中的一个条目。
这个类所代表的ACL入口基于RFC 3530中指定的ACL模型:网络文件系统(NFS)版本4协议。每个条目有以下四个组成部分:
类型组件确定条目是否授予或拒绝访问。
主体组件(有时称为“who”组件)是与条目授予或拒绝访问的标识相对应的用户主体
权限组件是一组权限
flags组件是一组标志，用于指示如何继承和传播条目
使用关联的AclEntry创建ACL项。构建对象通过调用它的构建方法。
ACL条目是不可变的，对于多个并发线程来说是安全的。
 */
...
public final class AclEntry {
     ...
     * @since 1.7
     * 一个抽象对象的构建器。
    通过调用AclEntry类定义的一个newBuilder方法来获得Builder对象。
    Builder对象是可变的，对于没有适当同步的多个并发线程来说不安全。
     */
    public static final class Builder {
         ...
         *          从该构建器的组件构造一个AclEntry。为了构造一个AclEntry，需要设置类型和who组件。
         */
        ...
        /**
         * Sets the principal component of this builder.设置该构建器的主要组件。
         *
         ...
         * permissions component of this builder is a copy of the given set.设置此构建器的权限组件。返回时，该构建器的权限组件是给定集合的副本。
         *
         ...
         * the given array.设置此构建器的权限组件。在返回时，该构建器的权限组件是给定数组中的权限的副本。
         *
         * @param   perms  the permissions component
         * @return  this builder
         */
         ...
         *          if the set contains elements that are not of type {@code
         *          AclEntryFlag}
         *          设置此构建器的flags组件。在返回时，该构建器的flags组件是给定集合的副本。
         */
         ...
         * @param   flags  the flags component
         * @return  this builder
         * 设置此构建器的flags组件。返回时，该构建器的flags组件是给定数组中的标志的副本。
         */
     ...
     * 构造一个新构建器。类型和谁组件的初始值为null。权限和标志组件的初始值是空集。
     */
    ...
    /**
     * Constructs a new builder with the components of an existing ACL entry.使用现有ACL入口的组件构造一个新的构建器。
     *
