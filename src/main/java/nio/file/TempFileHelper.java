 ...
 * initial attributes.帮助类，以支持创建具有初始属性的临时文件和目录。
 */
...
class TempFileHelper {
     ...
     * temporary directory if dir is {@code null}).在给定的目录中(如果目录为空，则在临时目录中)创建一个文件或目录。
     */
     ...
     * temporary directory if dir is {@code null}.在给定目录中创建临时文件，如果目录为空，则在临时目录中创建临时文件。
     */
     ...
     * temporary directory if dir is {@code null}.在给定目录中创建临时目录，如果dir为空，则在临时目录中创建临时目录。
     */
