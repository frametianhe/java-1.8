 ...
 * 一个字符缓冲区。
该类定义了字符缓冲区上的四种操作:
绝对和相对获取和放置读取和写入单个字符的方法;
相对块获取方法，将连续的字符序列从缓冲区传输到数组;和
将字符序列从字符数组、字符串或其他字符缓冲区传输到此缓冲区的相对散装方法;和
用于压缩、复制和分割字符缓冲区的方法。
可以通过分配(分配为缓冲区的内容分配空间)、将现有的字符数组或字符串包装到缓冲区中，或者通过创建现有字节缓冲区的视图来创建字符缓冲区。
与字节缓冲区一样，字符缓冲区也可以是直接的，也可以是非直接的。通过此类的wrap方法创建的char缓冲区将是非直接的。当且仅当字节缓冲区本身是直接的时，作为字节缓冲区的视图创建的字符缓冲区将是直接的。char缓冲区是否直接可以通过调用isDirect方法来确定。
这个类实现了CharSequence接口，以便可以在接受字符序列的地方使用字符缓冲区，例如在正则表达式包java.util.regex中。
这个类中没有返回值的方法被指定为返回被调用的缓冲区。这允许对方法调用进行链接。语句的顺序
例如，可以用单个语句替换吗
 */
    ...
    implements Comparable<CharBuffer>, Appendable, CharSequence, Readable
{
    ...
    // These fields are declared here rather than in Heap-X-Buffer in order to 这些字段在这里声明，而不是在heap - x缓冲区中声明
    // reduce the number of virtual method invocations needed to access these 减少访问这些对象所需的虚拟方法调用的数量
    // values, which is especially costly when coding small buffers. 值，这在编写小缓冲区时特别昂贵。
    //
    final char[] hb;                  // Non-null only for heap buffers 非空仅用于堆缓冲区
    final int offset;
    boolean isReadOnly;                 // Valid only for heap buffers 仅对堆缓冲区有效

    // Creates a new buffer with the given mark, position, limit, capacity, 用给定的标记、位置、限制、容量创建一个新的缓冲区，
    // backing array, and array offset 支持阵列和阵列偏移量
    //
     ...
     * rewinding of the buffer is performed.尝试将字符读入指定的字符缓冲区。缓冲区用作按原样存储字符的仓库:所做的惟一更改是put操作的结果。不执行缓冲区的翻转或回卷。
     *
     ...
     * is read-only.  </p>
     ...
     新缓冲区的内容将从该缓冲区的当前位置开始。该缓冲区内容的更改将在新缓冲区中可见，反之亦然;两个缓冲区的位置、限制和标记值是独立的。
     新缓冲区的位置将为零，它的容量和它的限制将是这个缓冲区中剩余的字符数，它的标记将是未定义的。当且仅当这个缓冲区是直接的，并且当且仅当这个缓冲区是只读的，新的缓冲区将是直接的。
     *
     * @return  The new char buffer
     */
     ...
     * only if, this buffer is read-only.  </p>创建一个新的char缓冲区来共享这个缓冲区的内容。
     新缓冲区的内容将是这个缓冲区的内容。该缓冲区内容的更改将在新缓冲区中可见，反之亦然;两个缓冲区的位置、限制和标记值是独立的。
     新缓冲区的容量、限制、位置和标记值将与此缓冲区的容量、限制、位置和标记值相同。当且仅当这个缓冲区是直接的，并且当且仅当这个缓冲区是只读的，新的缓冲区将是直接的。
     *
     * @return  The new char buffer
     */
     ...
     *
     * @return  The new, read-only char buffer创建一个新的只读char缓冲区来共享这个缓冲区的内容。
    新缓冲区的内容将是这个缓冲区的内容。该缓冲区内容的更改将在新缓冲区中可见;但是，新缓冲区本身是只读的，不允许修改共享内容。两个缓冲区的位置、限制和标记值是独立的。
     ...
     *          相对的get方法。在此缓冲区的当前位置读取字符，然后增加位置。
     */
     ...
     *          parameters do not hold
     *          相对批量get方法。
    这个方法将chars从缓冲区传输到给定的目标数组中。如果缓冲区中剩余的字符比满足请求所需的字符少，即如果长度>剩余()，则不会传输任何字符并抛出BufferUnderflowException。
    否则，该方法将这个缓冲区中的长度字符复制到给定的数组中，从该缓冲区的当前位置开始，并在数组中的给定偏移量处开始。然后，这个缓冲区的位置会随着长度的增加而增加。
    换句话说，这是对src表单方法的调用。get(dst, off, len)与循环具有完全相同的效果
    除非它首先检查这个缓冲区中是否有足够的chars，而且它可能更有效。
     */
     ...
     *          If this buffer is read-only
     *          相对批量放置方法(可选操作)。
    此方法将位于给定源缓冲区中的字符传输到此缓冲区。如果源缓冲区中剩余的chars比该缓冲区中的更多，也就是说，如果src.remaining() >存留()，则不会传输任何chars，并抛出BufferOverflowException。
    否则，该方法将n = src.残余()字符从给定缓冲区复制到此缓冲区，从每个缓冲区的当前位置开始。然后两个缓冲区的位置都增加了n。
     ...
     * method. </p>压缩此缓冲区(可选操作)。
     缓冲区当前位置与其限制(如果有)之间的字符被复制到缓冲区的开头。即char在指数p =位置()复制到指数为零,char指数p + 1复制指数,等等在char指数限制()- 1复制指数n =限制()——1 - p。然后将缓冲区的位置n + 1及其限制设置为它的容量。如果定义了标记，则丢弃该标记。
     缓冲区的位置设置为复制的字符数，而不是设置为0，这样，在调用此方法之后，可以立即调用另一个相对put方法。
     *
