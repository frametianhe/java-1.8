 ...
 * @since 1.7
 * 用于读取、写入和操作文件的异步通道。
当通过调用该类定义的打开方法之一打开文件时，将创建一个异步文件通道。该文件包含可读和可写的可变长度字节序列，并且可以查询其当前大小。当字节写入超过当前大小时，文件的大小会增加;当文件被截断时，它的大小会减少。
异步文件通道在文件中没有当前位置。相反，文件位置被指定给每个发起异步操作的读和写方法。CompletionHandler被指定为参数，并被调用以使用I/O操作的结果。该类还定义了启动异步操作的读和写方法，返回一个Future来表示操作的挂起结果。将来可用于检查操作是否已完成，等待其完成，并检索结果。
 */
    ...
    implements AsynchronousChannel
{
    ...
    options参数确定如何打开文件。读写选项确定是否应该打开文件进行读写。如果数组中不包含任何选项，则打开一个现有文件进行读取。
    除了读和写之外，还可以提供以下选项:
    TRUNCATE_EXISTING
    当打开一个现有文件时，文件首先被截断为0字节。当文件仅为读取而打开时，将忽略此选项。
    CREATE_NEW
    如果此选项出现，则会创建一个新文件，如果文件已经存在，则会失败。在创建文件时，检查文件是否存在，如果文件不存在，则对其他文件系统操作进行原子检查。当文件仅为读取而打开时，将忽略此选项。
    创建
    ...
    实现还可以支持其他选项。
    executor参数是ExecutorService，向它提交任务以处理I/O事件并为在结果通道上启动的操作分派完成结果。这些任务的性质是高度实现的，所以在配置执行器时应该小心。至少，它应该支持一个无限制的工作队列，不应该在execute方法的调用线程上运行任务。在通道打开时关闭执行程序服务将导致未指定的行为。
     ...
     *          read access if the file is opened for reading. The {@link
     *          SecurityManager#checkWrite(String)} method is invoked to check
     *          write access if the file is opened for writing打开或创建用于读取和/或写入的文件，返回一个异步文件通道来访问该文件。
    此方法的调用与调用的行为完全相同,其中opts是包含此方法指定的选项的集合。
    生成的通道与默认线程池相关联，将任务提交到该线程池来处理I/O事件，并将其分派给使用在生成通道上执行的异步操作的结果的完成处理程序。
     */
