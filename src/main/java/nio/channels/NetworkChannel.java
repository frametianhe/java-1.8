 ...
 * @since 1.7
 * 网络套接字的通道。
实现此接口的通道是网络套接字的通道。绑定方法用于将套接字绑定到本地地址，getLocalAddress方法返回套接字绑定到的地址，并使用setOption和getOption方法来设置和查询套接字选项。这个接口的实现应该指定它支持的套接字选项。
如果绑定和setOption方法没有返回值，则指定它们返回它们所调用的网络通道。这允许将方法调用链接起来。这个接口的实现应该专门化返回类型，以便可以链接实现类上的方法调用。
 */
//
